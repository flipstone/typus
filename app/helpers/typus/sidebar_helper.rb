module Typus
  module SidebarHelper
    ...
    def build_sidebar
      ...
      typus_user.application(app_name).sort {|a,b| a.typus_constantize.model_name.human <=> b.typus_constantize.model_name.human}.each do |resource|
      ...
      render "typus/helpers/sidebar/sidebar", :resources => resources
    end

    def default_actions(klass)
      Array.new.tap do |tap|
        tap << link_to_unless_current(Typus::I18n.t("Add new"), :action => "new") if typus_user.can?("create", klass)
    ...
    def custom_actions(klass)
      klass.typus_actions_on(params[:action]).map do |action|
        if typus_user.can?(action, klass)
