module Typus
  module TableHelper
    ...
    def build_table(model, fields, items, link_options = {}, association = nil, association_name = nil)
      render "typus/helpers/table/table",
             :model => model,
             :fields => fields,
             :items => items,
    ...
    def table_position_field(attribute, item, connector = " / ")
      ...
      [ [:move_to_top, "Top"],
        ...
        options = { :controller => "/typus/#{item.class.to_resource}", :action => "position", :id => item.id, :go => key }
    ...
    def table_boolean_field(attribute, item)
      ...
      options = { :controller => "/typus/#{item.class.to_resource}",
                  :action => "toggle",
                  :id => item.id,
