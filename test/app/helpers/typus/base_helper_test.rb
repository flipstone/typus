...
class Typus::BaseHelperTest < ActiveSupport::TestCase

  include Typus::BaseHelper
  ...
  context "login info" do
    ...
    setup do
      ...
      typus_user.stubs(:name).returns("Admin")
      typus_user.stubs(:id).returns(1)
    end

    should "skip rendering when we're using a fake user" do
      typus_user.stubs(:is_a?).with(FakeUser).returns(true)
      output = login_info
      assert_nil output
    end
    ...
    context "when the current user is not a FakeUser" do
      ...
      setup do
        typus_user.stubs(:is_a?).with(FakeUser).returns(false)
      end

      context "when the user cannot edit his informations" do

        should "render a partial with the user name" do
          typus_user.stubs(:can?).with('edit', 'TypusUser').returns(false)
          assert_equal ["typus/helpers/base/login_info"], login_info
        end
      ...
      context "when the user can edit his informations" do
        ...
        should "render a partial with a link" do
          link_options = { :action => 'edit', :controller => '/typus/typus_users', :id => 1 }

          typus_user.stubs(:can?).with('edit', 'TypusUser').returns(true)
          self.stubs(:link_to).with("Admin", link_options).returns(%(<a href="/typus/typus_users/edit/1">Admin</a>))
  ...
  context "header" do
    ...
    should "work returning a partial" do
      assert_equal ["typus/helpers/base/header"], header
    end
  ...
  context "display_flash_message" do
    ...
    should "be displayed" do
      ...
      expected = ["typus/helpers/base/flash_message",
