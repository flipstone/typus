...
class Typus::TableHelperTest < ActiveSupport::TestCase
  ...
  context "table_header" do
    ...
    should "work" do
      params = { :controller => "/typus/typus_users", :action => "index" }
    ...
    should "work with params" do
      params = { :controller => "/typus/typus_users", :action => "index", :search => "admin" }
      fields = TypusUser.typus_fields_for(:list)
  ...
  context "table_belongs_to_field" do
    ...
    should "work with associated model when user has access" do
      typus_user.expects(:can?).returns(true)
    ...
    should "work with associated model when user does not have access" do
      typus_user.expects(:can?).returns(false)
      comment = Factory(:comment)
      post = comment.post
