...
class Typus::SearchHelperTest < ActiveSupport::TestCase
  ...
  context "search" do
    ...
    should "reject controller and action params" do
      parameters = {"controller"=>"typus/posts", "action"=>"index"}
      expected = ["typus/helpers/search/search", {:hidden_filters => {}}]
    ...
    should "not reject applied filters" do
      ...
      expected = ["typus/helpers/search/search", {:hidden_filters=>{"published"=>"true", "user_id"=>"1"}}]
      assert_equal expected, search(Entry, parameters)
    end
