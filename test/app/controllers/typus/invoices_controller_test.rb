...
class Typus::InvoicesControllerTest < ActionController::TestCase
  ...
  #
  #   /typus/invoices/new?back_to=&order_id=4&resource=Order&resource_id=4
  #
  ...
  context "Unrelate" do
    ...
    setup do
      @invoice = Factory(:invoice)
      @order = @invoice.order
      @request.env['HTTP_REFERER'] = "/typus/orders/edit/#{@order.id}"
    end
